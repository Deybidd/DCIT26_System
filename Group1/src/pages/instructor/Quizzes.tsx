...
export default function InstructorQuizzes() {
  ...
  const [selectedQuizResponses, setSelectedQuizResponses] = useState<StudentDetail[]>([]);
  ...
  try {
    ...
    for (const q of res.data) {
      ...
      try {
        ...
        console.error(`Failed to fetch grading status for quiz ${quizId}:`, err);
              ...
              return (
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-sm font-regular mt-3 min-h-[40px]">
                ...
                <span className="font-bold text-lg">{quiz.questions.length} <span className="font-medium text-xs">items</span></span>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setShowResponsesFor(showResponsesFor === quizId ? null : quizId)}
                  ...
                  {needsGrading[quizId] && (
                    <>
                      <button
                        ...
                        className="bg-blue-300 border-1 border-black text-black font-semibold px-2 py-2 rounded text-xs hover:bg-blue-400"
                      >
                        View Details
                      </button>
                      <button
                        onClick={() => handleReleaseScore(quiz)}
                        className="bg-[#87FDA8]  border-1 border-black text-black font-semibold px-2 py-2 rounded text-xs hover:bg-green-400"
                      >
                        Release Score
                      </button>
                    </>
              ...
              <div className="space-y-4">
                {selectedQuizResponses.map((response: any, idx: number) => (
                  <div key={idx} className="border p-4 rounded bg-gray-50">
                    ...
                    <p className="font-semibold text-black">{response.student_name}</p>
                    <p className="text-sm text-gray-600">Year/Section: {response.yearSection}</p>
                    <p className="text-sm text-gray-600">Score: {response.correct_count}</p>
                    <p className="text-sm text-gray-600">Penalty Count: {response.tab_switches}</p>
                    <p className="text-sm text-gray-600">Submitted: {new Date(response.submitted_at).toLocaleString()}</p>
                  </div>
                ))}
              </div>
            )}
