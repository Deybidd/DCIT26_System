  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      ...
      try {
        ...
        const rate =
          maxPoints > 0
            ? Math.round((totalPoints / (maxPoints * 2)) * 100)
            : 0;
    ...
    <div className="ml-60 flex-col h-screen">
      ...
      <div className="flex-1 font-sans grid xl:grid-cols-2 lg:grid-cols-2 md:grid-cols-1 sm:grid-cols-1 justify-center gap-20 my-10 mx-30 p-4">
        ...
        <div className="flex flex-col text-black">
          ...
          <div className="flex flex-col bg-white rounded-2xl p-2 shadow-lg overflow-hidden w-full h-62 relative">
            <div className="flex text-sm justify-between px-5 pt-3 pb-2 border-b">
              <h1 className="font-bold">No. of Scheduled: {scheduledQuizzes.length}</h1>
              <a href="/instructor/quizzes" className="cursor-pointer hover:text-green-500 font-semibold">
                See More
              </a>
            </div>
                ...
                {scheduledQuizzes.slice(0, 3).map((q, index) => (
                  <li
                    ...
                    className={`grid grid-cols-1 gap-2 p-3 w-full rounded shadow-sm border-l-4 ${
                      index % 2 === 0 ? "bg-[#FFE6B1] border-[#87FDA8]" : "bg-[#FFA0E4] border-pink-400"
                    }`}
                  >
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-xs text-gray-600">Subject: {q.subject_code}</span>
                    </div>
                  </li>
                ))}
              </ul>
        ...
        <div className="flex flex-col text-black">
          ...
          <div className="flex flex-col bg-white rounded-2xl p-2 shadow-lg overflow-hidden relative h-62">
            <div className="relative">
              <ul className="grid grid-cols-1 gap-2 p-3">
                {reminders.slice(0, 3).map((r, i) => (
                  <li key={i} className="flex items-center gap-4">
                    <svg width="32" height="32" viewBox="0 0 41 41" fill="none" xmlns="http://www.w3.org/2000/svg" className="shrink-0">
