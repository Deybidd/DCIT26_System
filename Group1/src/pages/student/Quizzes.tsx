				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
					{quizzes.map((quiz) => (
						<div
							...
							className={`border-2 ${
								quizColors[quiz.id]
							} p-5 rounded-xl shadow border text-black relative flex flex-col justify-between`}
						>
							<div>
									...
									<div className="mt-2 flex items-center gap-2">
										...
										{studentScores[quiz.id] !== undefined && (
											<span className="text-sm font-semibold text-green-700">
												Your Score: {studentScores[quiz.id]} /{" "}
								...
								<div className="absolute top-2 right-2 flex flex-col items-end gap-2">
									<button
										...
										className="text-xs bg-[#87FDA8] border px-2 py-1 rounded shadow hover:bg-green-400 cursor-pointer"
									>
										View Attempts
									</button>
									{!quiz.graded && (
								...
								{showAttemptsModal === quiz.id && (
									<>
										...
										<button
											...
											className="absolute top-8 left-7 border-2 border-black z-50 bg-red-400 font-bold text-black px-4 py-1 rounded-lg shadow hover:bg-red-500 cursor-pointer"
										>
											<svg
												width="20"
												height="20"
										...
										<div className="absolute top-11 right-4 z-40 w-80 bg-[#87FDA8] border-2 border-black rounded-lg p-4 shadow max-h-58 overflow-y-auto">
											...
											{attemptHistory.length === 0 ? (
												...
												attemptHistory.map((a, idx) => (
													<div
														key={idx}
														className="border-1 py-3 text-sm bg-white p-3 rounded-lg mt-1 flex justify-between"
													>
														<div>
														...
														<div className="font-semibold">
															{a.released ? (
																<>
																	...
																	<span className="text-green-600 ml-2 text-sm font-extrabold">
																		({a.percentage}%)
																	</span>
																</>
															) : (
							...
							<button
								...
								className={`px-4 py-2 rounded mt-3 ${
									attemptInfo[quiz.id]
										? "bg-[#87FDA8] border-2 border-black rounded-lg text-black cursor-pointer hover:bg-green-400"
